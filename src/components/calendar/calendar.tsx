...
export class Calendar {
  ...
  render() {
    ...
    if (this.selectedDay) {
      toolbarEventButtons = (
        <div class="bc_toolbar-end">
          <button
            class="bc_toolbar-action-button bc_button-success"
            onClick={() => this._onAddEvent()}
          >
            Add event
          </button>
          {this.selectedDay.hasEvent ? (
            <span>
              <button
                class="bc_toolbar-action-button bc_button-success"
                onClick={() => this._onReplaceEvent()}
              >
                {' '}
                Replace event
              </button>
              <button
                class="bc_toolbar-action-button bc_button-danger"
                onClick={() => this._onRemoveEvent()}
              >
                Remove all events
              </button>
            </span>
    ...
    if (this.showToolbar) {
      toolbarTemplate = (
        <div>
          <div class="bc_calendar-toolbar-top">
            <div class="bc_toolbar-start">
              <button class="bc_toolbar-button" onClick={() => this.selectToday()}>
                Today
              </button>
            </div>
            {toolbarEventButtons}
          ...
          <div class="bc_calendar-toolbar-bottom">
            <select class="bc_selection-select" onChange={(event: UIEvent) => this._changeSelection(event)}>
    ...
    calendar = (
      <div class="bc_calendar-container">
        <div class="bc_calendar-header">
          <div class="bc_header-left" onClick={() => this._showPreviousMonth()}>
            <button class="bc_nav-button-left"></button>
          </div>

          <div class="bc_header-year">
            <select
              class="bc_header-month-select"
              onChange={(event: UIEvent) => this._changeMonth(event)}
            >
              {months}
            </select>
            <input
              class="bc_year-input-number"
              type="number"
              value={this.year}
          ...
          <div class="bc_header-right" onClick={() => this._showNextMonth()}>
            <button class="bc_nav-button-right"></button>
          </div>
        </div>

        <div class="bc_calendar-days bc_calendar-days-name">
          {this.languages[this.languageCode || 'en']['day_of_week_short'].map(
            dayName => <div class="bc_day-name">{dayName}</div>
          )}
        </div>

        <div class="bc_calendar-days">
          {this.days.map(date => (
            <div
              class={
                (this.days.indexOf(date) == this.dayIndex || date === this.selectedDays[0] ? 'bc_active-day' : '')
                +
                (date === this.selectedDays[1] ? 'bc_range-end-day' : '')
              }
              onClick={() => this.selectionType !== 'range' ? this._selectDay(date) : this._selectMultiple(date)}
            >
              <span
                class={
                  (date.month !== this.month + 1 ? 'bc_muted-day' : '') +
              ...
              >
                {date.day}
                <span class={date.hasEvent ? 'bc_day-event-dot' : ''} />
              </span>
            </div>
          ))}
        </div>
    ...
    datePicker = (
      <div class="bc_date-picker">
        <div class="bc_input-with-icon" onClick={() => this._switchCalendarBool()}>
          <input
            type="text"
            value={this.pickedDate}
            disabled={true}
            class="bc_date-picker-input" />
          <i class="bc_calendar-icon" />
        </div>
