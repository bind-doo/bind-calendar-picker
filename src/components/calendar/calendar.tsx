...
export class Calendar {
	...
	private languages = {
		...
		bs: {
			...
			day_of_week: ['Ponedjeljak', 'Utorak', 'Srijeda', 'Četvrtak', 'Petak', 'Subota', 'Nedjelja'],
	...
	private _getCalendarDays(year: number, month: number): Array<Day> {
		...
		const totalDisplayDays: number = Math.ceil((totalDaysInCurrentMonth + firstDayOfCurrentMonth) / 7) * 7;
		let days: Array<Day> = [];

		let previousMonthLastDay: number = prevMonthEnd.getDate() - firstDayOfCurrentMonth;
		let firstDayOfNextMonth: number = 0;

		for (let dayCounter = 0; dayCounter < totalDisplayDays; dayCounter++) {
			const isNextMonth: boolean = dayCounter > totalDaysInCurrentMonth + firstDayOfCurrentMonth - 1;
			const day: Day =
				dayCounter < firstDayOfCurrentMonth
					? {
						day: ++previousMonthLastDay,
						month: this.month == 0 ? 12 : this.month,
						year: this.month == 0 ? date.getFullYear() - 1 : date.getFullYear(),
						hasEvent: false,
						data: []
					}
					: {
						day: isNextMonth ? ++firstDayOfNextMonth : dayCounter - firstDayOfCurrentMonth + 1,
						month: isNextMonth ? this.month == 11 ? 1 : this.month + 2 : this.month + 1,
						year: isNextMonth ? this.month == 11 ? date.getFullYear() + 1 : date.getFullYear() : date.getFullYear(),
						hasEvent: false,
						data: []
					};
	...
	private _getTodayEvents(today: Day): Array<Day> {
		return this.events.filter(event => event.day == today.day && event.month == today.month && event.year == today.year);
	}
	...
	private _getDayIndex(days: Array<Day>, day: Day): number {
		...
		days.forEach((dayOfMonth, idx) => {
			if (dayOfMonth.day == day.day && dayOfMonth.month == day.month && dayOfMonth.year == day.year) index = idx;
		});
		return index;
	}
	...
	render() {
		...
		this.languages[this.languageCode].months.forEach(month => {
			if (month === this.languages[this.languageCode]['months'][this.month]) {
				months.push(
		...
		calendar = (
			<div class="bc-calendar-container">
				<div class="bc-calendar-header">
					<div class="bc-header-left" onClick={() => this._showPreviousMonth()}>
						<button class="bc-nav-button-left">
							<i class="bc-arrow-icon bc-arrow-left" />
						</button>
					</div>
					...
					<div class="bc-header-right" onClick={() => this._showNextMonth()}>
						<button class="bc-nav-button-right">
							<i class="bc-arrow-icon bc-arrow-right" />
						</button>
					</div>
				</div>

				<div class="bc-calendar-days bc-calendar-days-name">
					{this.languages[this.languageCode || 'en']['day_of_week_short'].map(dayName => (
				...
				<div class="bc-calendar-days">
					{this.days.map(date => (
						<div class={'bc-day-container ' + ((this.days.indexOf(date) == this.dayIndex || date === this.selectedDays[0]) && this.selectedDays.length <= 1 ? 'bc-active-day' : '') + (this._checkIsDateInRange(date, this.selectedDays) ? 'bc-active-day' : '')} onClick={() => (this.selectionType !== 'range' ? this._selectDay(date) : this._selectMultiple(date))}>
							<span class={(date.month !== this.month + 1 ? 'bc-muted-day' : '') + (this._isCurrentDay(date) ? 'bc-current-day' : '') + (date.month === this.month + 1 && !this._isCurrentDay(date) ? 'bc-day-number' : '')}>
								{date.day}
		...
		datePicker = (
			<div class="bc-date-picker">
				<div class="bc-input-with-icon" onClick={() => this._switchCalendarBool()}>
					<input type="text" value={this.pickedDate} disabled={true} class="bc-date-picker-input" />
